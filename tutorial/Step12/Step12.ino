/**************************************************************************
 ...
 Description:   This is the step #12 for building a WWVB Clock.
                In this sketch, time is displayed in a more
                colorful, seven-segment font.
...
#include <TFT_eSPI.h>
#include <TimeLib.h>
#define TITLE "WWVB Step #12: ALMOST A VFD"
...
void showDate() {
  ...
  if (timeStatus()!=timeSet)                       // has date been set yet?
    tft.drawString("Waiting for Signal",x,y,f);    // no
...
void startNewFrame()
{
  ...
  tft.fillRect(20,140,200,40,TFT_BLACK);           // clear previous time
  showTime();
  showDate();
}
...
void clearSegments() {
   for (int i=0; i<6; i++)                         // wipe segment data
     seg[i] = -1; 
}
